...
\begin{abstract}
    ...
    \replaced{onto a porous substrate}{in this context}.
    We identify three categories of adsorption based on 
    the degree of dehydration of the ion, 
    viz., steric adsorption \added{corresponding to a lack of dehydration}, 
    imperfect adsorption \added{with partial dehydration}, and 
    perfect adsorption \added{representing total dehydration}.
    ...
    anions, \cl, always adsorb sterically 
    \deleted{, without dehydration}. Among cations, the divalent
    \mg does not dehydrate, and is \added{also} adsorbed sterically. 
    On the other hand, \na \deleted{and \li} 
    adsorbed to a large fraction perfectly \deleted{with complete dehydration of the ion} 
    \replaced{and \li exhibits a significant fraction of imperfectly adsorbed ions,}
    {In addition, \li to a certain extent exhibits
    imperfect adsorption characterized by partial dehydration}.
...
\replaced{This mean-field approximation predicts accumulation
of ions near a surface due to the mutual electrostatic repulsion
of the ions and entropic factors.}{This mean-field approximation, due to mutual electrostatic repulsion 
among the ions and entropic factors,
predicts accumulation of ions near a surface.}
\replaced{Deviations in ion distribution from the predictions of PB theory near a substrate are
    the defining characteristic of the specific adsorption phenomenon
~\cite{stern:1924:theory,grahame:1947:electrical}.}{Often, any deviations in ion 
distribution, near the substrate, from PB theory are
attributed to the specific adsorption phenomenon
...
\deleted{In this work, we distinguish the specifics of the adsorption of \na as opposed to \li 
by the degree of their dehydration.} 
Further, our previous work on divalent ions
exhibited that \mg maintains its 
hydration \added{structure} regardless of where the ion is located in the 
lipid bilayer~\cite{kruczek:2019}, yet maintaining 
a distribution distinct from that
predicted by PB theory. Hence, \replaced{in this work
    we characterize different modes of adsorption corresponding
to different ions. Here we}{we feel it is necessary to} categorize the 
...
In \replaced{the somewhat different}{the} 
...
being diffuse \added{(similar to our steric adsorbed case)}, and the site-bound ions are further characterized
by outer-shell \added{(again analogous to our steric adsorption ions)} 
or inner shell binding 
\added{(analogous to the imperfect or perfect adsorbed ions)}, depending on
...
\added{always} \emph{steric adsorption}.
...
\added{Bilayers are constructed of 200 lipids, with 100 lipids per leaflet.}
...
\added{yet} statistically viable.
Systems with \mg are simulated with twice the number of 
anions to counter the +2 charge of the cation. All the\deleted{ new} systems are simulated for 1~$\mu$s of simulated time. 
...
\added{All the} simulations are performed using the GROMACS molecular dynamics software package, 
...
We have \replaced{computed}{done this by computing} 
the interaction cross-term for the \mg ion from Grotz \etal
with SPC/E water explicitly, using \replaced{the Lorentz-Berthelot mixing rules.}{the Lorentz-Berthelot mixing rule and 
using this without changing the
self-term for the \mg ion.}
...
yield a scattering form-factor\added{. Densities can be obtained from the form-factor by solving
the inverse problem, which is a technically hard problem. 
In experiments this is usually solved by fitting a model to the form-factor.}
\deleted{, and by applying a model one can solve the inverse problem
to determine the corresponding scattering-length density.} Simulations give 
us direct access to atomic positions, and consequently
the densities. This allows us to compute a 
scattering form-factor by taking a cosine transform of the density\replaced{. The computed
form-factor}{, which}
...
of the bilayer in the \li \replaced{simulation}{systems} above that seen in the \na \replaced{simulation}{system}.
The \mg simulations \replaced{, irrespective of the parameter set, yield}{report} much less thickening than \added{the} \deleted{seen in} 
Li\textsuperscript{+} \added{simulation.}\deleted{, with little variation
between the different parameter sets.} \deleted{Turning to the \db, the \li systems are 
significantly thicker than the \mg systems, with the least thickened
bilayer resulting from the \mbnbfixmicro parameters.} 
\added{The volumes per lipid (\vl), headgroup (\vh), and chains (\vc) are computed using the method of
...
\deleted{which is to be expected -- this should be intrinsic to the lipid being studied and not depend
on the solution or ion parameter set.}
The two-dimensional area per lipid \al~is defined as
\replaced{$\frac{2V_c}{2D_c}$ as is often reported from SAXS and SANS experiments~\cite{nagle:2000}, and is an important
measure of how the lipids condense as the bilayer thickens.}
{$\frac{V_c}{D_c}$. This value is often reported from SAXS and SANS experiments, and is an important
measure of how the lipids condense as the bilayer thickens.}
\replaced{Both the simulations with \mg yield bilayers with a larger \al~ 
    than the monovalent ions studied in this work, and are closer
in area to the simulation without salt.
}{The \mgmbnbfix forms a bilayer with an \al~closer that than that of \li, and
the \mbnbfixmicro
simulation has a significantly larger \al~than the other systems studied, and much closer
to that of the no-salt bilayer.}
...
\deleted{Experiments can be performed on lipids labeled with NMR responsive isotopes to quantify the 
chemical environment of atoms or the organization and dynamics of quadrupole moments.}
\replaced{
At present, these experiments with various salts are sparse.
}{Much of the available data are of measurements taken using lipids labeled with deuterium at 
various positions along the molecule, including
some headgroup carbons, and the acyl chains. These data are obtained without salt in the system. %~\cite{....} 
More data including the effects of salts are needed to validate our simulation results. 
Further, heavy atom NMR studies are needed to ascertain the environment of lipid headgroups and ions.}
\added{Thus,} we report these data with anticipation that future experiments will fill this gap and validate
or invalidate
these numbers.
...
with \na and Li\textsuperscript{+}, which \replaced{is}{would be} consistent with the slight thickening
...
In simulations the interface is not a simple surface, but a region \added{without a clear point of delineation}. \deleted{Thus, making it difficult to delineate 
the ``slip-surface''.}

\subsubsection{Hydration boundary}
\deleted{Thus, w}We \replaced{identify}{define} this 
slip-surface boundary as the point where 
water orientational ordering is negligible, i.e.
beyond the ``slip-surface'' 
boundary water \replaced{quadrupoles}{dipoles} 
are \replaced{sufficiently isotropic,
giving dielectric properties of water similar to that of bulk solvent
}{sufficiently randomly oriented}.
...
roughly describes the organization of the quadrupole moments of water, \replaced{and the value of this parameter
can be used to compute the }{and the sum over the 
histogram bins weighted by the number of water in each slice can be related 
to the} quadrupolar splitting values determined in deuterated water NMR experiments~\cite{aaman:2003,kruczek:2017:ether}.
The vertical dotted lines in figure~\ref{fig:h2order} denote regions of interest in the bilayer 
based on the \replaced{sign}{directionality} of the second order parameter. We call the innermost region of negative ordering
...
\deleted{Again, the monovalent ions have less organized water than the no-salt case, perturbing the
water organization when compared to the no-salt simulation.}
\deleted{This is distinct from the \mg simulations where waters are much more 
organized than the no-salt case.}
...
The lengths of the solvent occupied regions, $D$, \replaced{in each system is}{in each of the two systems are} found by measuring the distance across the solvent from the 
...
These values can be seen in table~\ref{tab:gctheory}. \added{Since our
...
We also compare \replaced{the}{other} 
...
\replaced{is related to}{corresponds with} 
...
of \cl anions adsorbed can be seen in \replaced{supporting table S2}{supporting table ??}.
...
These data \replaced{are}{is} histogrammed and averaged over
...
water from the shell. \added{We use the number of coordinating waters of an ion in the bulk solvent region
...
\replaced{the number of imperfectly adsorbed ions}{this} by counting the number of ions with 
...
compared to the \mgmicro model\added{,}\replaced{resulting in}{ -- thus there is} a stronger electric field 
...
adsorption to the surface \added{(figure 8)}. This affect appears irrespective of the force-field 
used in the case
of \mg, which primarily adsorbs in the non-Langmuir type steric modality.
\deleted{We note that ions that are more hydrated tend to leave the bilayer structure as 
observed using x-ray scattering relatively unaltered.}
...
scattering,
or various NMR methods \added{(figures 1, 2, and 3 respectively).}
While the effect on lipid bilayer structure is not obvious in
the electron density \added{(figure 1)}, the perturbation can be seen in the \db~and water density --
the less hydrated ions induce slight thickening of the lipid bilayer. This
is reinforced by the chain ordering, where these ions increase chain ordering \added{(figure 2)} 
...
\replaced{larger}{smaller} 
for the less hydrated ions \na and \li when compared to the more hydrated Mg\textsuperscript{2+}. \added{(figure 3)} 
...
\begin{table}
    ...
    \db~ is a distance between the Gibb's surfaces\added{\cite{fogarty:2015}} on the probability density of solvent as it approaches the lipid bilayer.
...
\added{as was done in Saunders \etal 2019~\cite{saunders:2019}}.}
    \label{tab:struc}
    {\tiny
...
\begin{table}
    \caption{Fractions per lipid of cations perfectly adsorbed, imperfectly adsorbed, sterically adsorbed, and non-adsorbed cations
        \added{averaged over the last 150~ns of simulation time}. These are computed
    ...
    ions. The remaining are considered sterically adsorbed. \deleted{We report these numbers as a fraction 
    of ions per lipid of the total number of ions in the system.}
...
\begin{figure}
    \caption{Comparison of x-ray scattering formfactors (a,b) and associated electron densities (c,d) for simulated systems. 
        \deleted{We see little change in the scattering formfactor (a)
    electron density for the \nambnbfix system (c). \li results in a bilayer that is only slightly thickened when compared to the no-salt system. }
    \added{The system with \li salt has a slightly thicker bilayer compared to \na and the simulation without salt (a,c) and},
...
\clearpage
\begin{figure}
    \caption{Water order parameters. \deleted{We see that for the monovalent cations}  
        \replaced{The P1 and P2 calculated for monovalent cations (a,c) show greater 
        ...
        }{(a,c) there is significantly less organization of the waters in all the systems compared to the no-salt system in
        both P1 and P2. \na and \li have similar effects on the water organization in all parts of the lipid
    bilayer.} \deleted{The \mg systems { (d)} {report} {a much} stronger ordering in P2 , 
with the most organization of the bilayer waters in the \mbnbfixmicro simulation (cyan).}
\added{On the hand, in the presence of \mg salts we observe an overall less pronounced effect in the
