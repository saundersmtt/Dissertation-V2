...
Ion-ion and ion-water interactions were described using Joung and Cheatham~\cite{joung:2008} parameters, 
...
cooperativity effects, such as polarization depend \deleted{strongly} 
...
\begin{table}[h!tb] 
    ...
    \begin{tabularx}{\textwidth}{Y|Y|Y|Y} Cluster Size & Water (kJ/mol)& MeAc (kJ/mol) & DePh (kJ/mol)\\\hline 
...
this nature may not have a unique solution; thus, 200 independent optimizations are performed,
...
\begin{figure}[h!tb]
    ...
    \caption[Representative Nelder-Mead (NM) optimization run.]{(a) Representative Nelder-Mead (NM) optimization run. Each point
    ...
    Nelder-Mead algorithm and available moves~\cite{fogarty:2014:paropt}). The insert illustrates the 
...
\begin{figure}[h!tb]
    \includegraphics[height=0.5\textheight]{figure_1.eps}
...
improved the relative substitution between the clusters of various sizes. \end{swaplandscape}
...
result corresponds with the smaller bilayer thickness in the optimized system.
    ...
    \label{figch2:op}
    \includegraphics[height=0.5\textheight]{figure_s2.eps}
\end{figure}
...
Headgroup carbon order parameters are also used to examine ion-lipid interaction in the headgroup. 
Results for the $S_{CD}$ from our simulations can be seen in Table~\ref{tab:scd}, computing using the method of 
Egberts and Berendsen~\cite{egberts:1988}. We note that the
$\Delta |S_{CD}|$ from our simulations with both original LB rules as well as MB-NBfix parameters
stay quite close to zero, although the sign of the $S_{CD}$ is opposite that for the system without salt
in both cases. Since experimentally the sign is not known in systems with salt, and is assumed
from indirect measurements~\cite{seelig:1987}, it is difficult to assign a definitive value. Thus, we report
the absolute value of the order parameter.
\begin{table}[h!tb]
\centering
\caption{$S_{CD}$ Order parameters for $\beta$ (C5) and $\alpha$ (C6) carbons.}
\begin{tabular}{lcc}
\toprule
& $\beta$ & $\alpha$ \\
\midrule
No Salt & $-0.0574 \pm 0.0098$ & $-0.1108 \pm 0.0115$ \\
Na$^+$ LB Rules & $0.0157 \pm 0.0095$ & $0.0286 \pm 0.0079$ \\
$\Delta |S_{CD}|$ & $0.0417$ & $-0.0822$ \\
Na$^+$ MB-NBfix & $0.0555 \pm 0.0068$ & $0.0976 \pm 0.0074$ \\
$\Delta |S_{CD}|$ & $-0.0019$ & $-0.0132$ \\
\bottomrule
\end{tabular}
\label{tab:scd}
\end{table}
This is consistent with the result from the work by Akutsu \etal~\cite{akutsu:1991}, who reported that the change in the 
headgroup $S_{CD}$ of POPC with the addition of \nacl salt is insignificant.
...
\begin{figure}[h!tb]
    \includegraphics[height=0.3\textheight]{figure_3.eps}
...
center. Compared to the system simulated with LB rules (a), the system simulated with optimized cross terms (b)
    ...
    \label{figch2:cood}
    \includegraphics[height=0.5\textheight]{figure_5.eps}
\end{figure}
...
headgroup, and are expected to be less affected by the new parameter set.}
    ...
    \includegraphics[height=0.5\textheight]{figure_7.eps}
\end{figure}
...
Einstein's relation for 2D diffusion 
\begin{equation}
...
for clarity.\ (b) and (d)
    ...
    \label{figch2:gouy}
    \includegraphics[height=0.5\textheight]{figure_s4.eps}
\end{figure}
